(function attachStepDefinitions(root, factory) {
  ...
  function createCockpitToolsSessionImportTail(startId, startOrder) {
    const id = Number(startId) || 10;
    const order = Number(startOrder) || id * 10;
    return [
      {
        id,
        order,
        key: 'cockpit-tools-session-ready',
        title: '等待 ChatGPT Plus Session',
        sourceId: 'chatgpt',
        driverId: 'background/cockpit-tools-session-import',
        command: 'cockpit-tools-session-ready',
      },
      {
        id: id + 1,
        order: order + 10,
        key: 'cockpit-tools-session-import',
        title: '导入当前 ChatGPT 会话到 cockpit-tools',
        sourceId: 'cockpit-tools-panel',
        driverId: 'background/cockpit-tools-session-import',
        command: 'cockpit-tools-session-import',
      },
    ];
  }
  ...
  function resolvePlusSessionImportTail(options = {}, signupMethod = SIGNUP_METHOD_EMAIL) {
    ...
    if (strategy === PLUS_ACCOUNT_ACCESS_STRATEGY_COCKPIT_TOOLS_SESSION) {
      return createCockpitToolsSessionImportTail;
    }
    return null;
  }
  ...
  const PLUS_PAYPAL_COCKPIT_TOOLS_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_PAYPAL_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_PAYPAL_HOSTED_CHECKOUT_COCKPIT_TOOLS_SESSION_STEP_DEFINITIONS = createHostedCheckoutSteps(
    PLUS_PAYPAL_HOSTED_CHECKOUT_PREFIX_STEP_DEFINITIONS,
    7,
    70,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_GOPAY_COCKPIT_TOOLS_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_GOPAY_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  const PLUS_GPC_COCKPIT_TOOLS_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    PLUS_GPC_PREFIX_STEP_DEFINITIONS,
    10,
    100,
    SIGNUP_METHOD_EMAIL,
  ...
  function getOpenAiModeStepDefinitions(options = {}) {
    ...
    if (shouldTreatHostedCheckoutAsFinalStep({
      ...
      if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_COCKPIT_TOOLS_SESSION) {
        return PLUS_PAYPAL_HOSTED_CHECKOUT_COCKPIT_TOOLS_SESSION_STEP_DEFINITIONS;
      }
