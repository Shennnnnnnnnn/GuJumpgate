...
test('sidepanel exposes FlowPilot-style SMS settings and phone signup controls', () => {
  ...
  assert.equal(/#phone-verification-section\s*\{[\s\S]*?display:\s*none\s*!important/i.test(css), false);
});
...
test('sidepanel wires SMS signup settings into capability and step-definition updates', () => {
  ...
  assert.match(sidepanelSource, /rebuildStepDefinitionState\(nextPlusModeEnabled,\s*\{[\s\S]*panelMode:\s*nextPanelMode/);
  assert.match(sidepanelSource, /getSelectedPanelMode\(\)/);
  assert.match(sidepanelSource, /updatePanelModeUI\(\);\s*updatePhoneVerificationSettingsUI\(\);/);
});
